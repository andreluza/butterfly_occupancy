...
#     Interpretation Empirical data analyses - L dispar
...
# load output (batch) ----------------------------------
list_output <- list.files (here ("model_output", "empirical"),pattern = "_Lycaena*") 
...
table_summ <- lapply(list_output, function (ng){
      ...
      #run the function for beta parameters
      # occupancy coeffs
...
tab_kable<-table_summ %>%
  ...
  filter (Model == "outputNNG15_Lycaena disp.Rdata") %>%
  # bind results with informative priors
  cbind (table_summ %>%
           dplyr::select (Average, LCI,UCI,RHat,percent_PPO, Model) %>%
           filter (Model == "outputNNG15InfPrior_Lycaena disp.Rdata"))
...
knitr::kable(tab_kable [,-which(colnames(tab_kable) == "Model")],
             ...
             caption = "Estimated parameters of the multi-season site-occupancy model fitted to L. dispar data.", booktabs = T) %>%
  add_header_above(c("-"=1,"Weak prior, NG=15" = 5, "Informative prior, NG=15" = 5))


# end
# plot(plogis(-1.116 + (-0.150*seq(-1.5,1.5,0.1))+(-1.459 *seq(-1.5,1.5,0.1)^2)))
