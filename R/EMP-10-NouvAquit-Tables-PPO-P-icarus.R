...
#     Analysis at Nouvelle-Aquitaine scale 
...
# load output (batch) ----------------------------------
list_output <- list.files (here ("model_output", "empirical"),pattern = "_Polyommatus*")
list_output <- list_output[-grep("buffer", list_output)]

# load water  -------------------------------
load (file = here ("Processed_data",
                   "Water.RData"))

# Create Table of coefficents / prior posterior overlap assessment
#load package
...
table_summ <- lapply(list_output, function (ng){
      ...
      sigma_sq_overlap <- MCMCtrace(out$theta.samples,
                                 ...
                                 pdf=F,
                                 type = "density")
      sigma_sq_overlap <- sigma_sq_overlap[which(sigma_sq_overlap$param == "sigma.sq"),] # select the parameter (the inverse gamma prior don't apply to all theta)
      # sigma_sq_T
      sigma_sq_T_overlap <- MCMCtrace(out$theta.samples,
                                    ...
                                    pdf=F,
                                    type = "density")
      sigma_sq_T_overlap <- sigma_sq_T_overlap[which(sigma_sq_T_overlap$param == "sigma.sq.t"),] # select the parameter (the inverse gamma prior don't apply to all theta)
      
      
      # phi
      # be flexible to consider different priors (weakly informative and informative)
...
tab_kable<-table_summ %>%
  ...
  filter (Model == "outputNNG15_Polyommatus .Rdata") %>%
  # bind results with informative priors
  cbind (table_summ %>%
           dplyr::select (Average, LCI,UCI,RHat,percent_PPO, Model) %>%
           filter (Model == "outputNNG15InfPrior_Polyommatus .Rdata"))
