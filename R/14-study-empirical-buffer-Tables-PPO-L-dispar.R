...
table_summ <- lapply(list_output, function (ng){
      ...
      # load output
      load (file=here ("model_output", "empirical",ng)) # call from the list
      ...
      # Phi
      # be flexible to consider the different priors used (weakly informative and informative)
...
# melt (transform the list into a data frame)
...
tab_kable<-table_summ %>%
  ...
  filter (Model == "buffer_outputNNG15_urban_Lycaena disp.Rdata") %>%
  # bind results with informative priors
  cbind (table_summ %>%
           dplyr::select (Average, LCI,UCI,RHat,percent_PPO, Model) %>%
           filter (Model == "buffer_outputNNG15InfPrior_urban_Lycaena disp.Rdata"))
