...
#     Interpretation Empirical data analyses
#     Analysis for the species
#     "Coenonympha oedippus (Fabricius, 1787)",  "Euphydryas aurinia (Rottemburg, 1775)", "Lycaena phlaeas (Linnaeus, 1761)","Maniola jurtina (Linnaeus, 1758)" 
#     Within the buffer of 10 km^2 around Bordeaux
...
# load output (batch) ----------------------------------
list_call <- c("Coenonympha","Maniola","Lycaena phla","Euphydryas") # list of names to be used to call the results

# call the 
list_output <- list.files (here ("model_output", "empirical"),pattern = "buffer_outputNNG15_urban") 
(list_output <- list_output[-c(3,6)]) # remove species shown in the main text
...
knitr::kable(tab_kable [,-which(colnames(tab_kable) == "Model")],
             ...
             format="latex",
             digits = 3, 
             col.names = c("Parameter","Mean","LCI","UCI","RHat","Overlap"),
             caption = "Estimated parameters of the multi-season site-occupancy model fitted to data of four butterfly species.", booktabs = T)
