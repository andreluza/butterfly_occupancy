...
#     Modeling the distribution of "Coenonympha oedippus (Fabricius, 1787)",  "Euphydryas aurinia (Rottemburg, 1775)", "Lycaena phlaeas (Linnaeus, 1761)","Maniola jurtina (Linnaeus, 1758)" using data from 2000-2023
...
lapply (sp, function (sel_sp) {
      ...
      print(c(table(data.list.full$y>0) ,
              sum(apply (data.list.full$y,1,max,na.rm=T)))
      )
      
      #naive yearly occupancy
      print(unlist(lapply (seq(1,ncol(data.list.full$y)), function (i)
      ...
      res_plots <- lapply (list (NG15weak), function (out) {
        ...
        #summ_tab_psib<-lapply (seq(1,nrow(out.pred.occ$z.0.samples)), function (i)
        
        #  colSums(out.pred.occ$z.0.samples[i,,])/ncol(out$out$psi.samples)
        ...
        p3<- data.frame (psi = apply(summ_tab_psi,2,mean),
          ...
          geom_line(data = data.frame (psi = apply (sp_table_years[,,,sel_sp],2,sum,na.rm=T)/nrow(sp_table_years),year = seq(2000,2023)), 
                    aes (x=year, y=psi))+
          ggtitle("")+
      ...
      png (here ("figures", "empirical",
                 paste ("Fig_summ_results_", substr(sp_list[sel_sp],1,12), "_buffer.png")),
                 width = 28,height = 10,units = "cm",res=200)
      ...
      # plot and save results
      png (here ("figures", "empirical",
                 paste ("estimates_missing_cells_", substr(sp_list[sel_sp],1,12), "_buffer.png")
                 ),width = 12,height = 14,units = "cm",res=150)
      
      grid.arrange(# weak prior
        res_plots[[1]]$p1_missing,
