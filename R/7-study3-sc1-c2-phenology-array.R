...
#               Scenario phenology + observer preferences for mid season
...
# SCENARIO phenology - SCENARIO 0 + SCENARIO 1 + SCENARIO 2 + SCENARIO 3 + phenology & observer preferences for mid season & J=10

# using the sampling scheme created in the code '7-study3-sc1-c1-Sampling-Design-Phenology.R'
...
for (s in 1:n.sims) {
  ...
  for (sc in 1:n.scenarios) {
    ...
    occ.covs <- list(int = X[, , 1],
                     occ.cov.1 = X[, , 2])
    det.covs <- list(int = V_itj_L_i[, , , 1],
                     det.cov.1=V_itj_L_i[, , , 2],
                     det.cov.2=V_itj_L_i[, , , 3]
    )
