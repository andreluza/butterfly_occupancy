...
# create dirs to receive the figures ----------------------
dir.create("figures")
dir.create(here("figures", "sims_D&S"))
...
load(file = here ("model_output", "output_simulations", "sims_D&S",
                  "sim-mixed-stPGOcc-results-SimSce1600.rda"))
...
# project one data set of D&S
rep.indx <- matrix(ifelse(rbinom(J * n.time, 1, 0.1) == 1, 2, 1), J, n.time)

# Site x Replicate
y_mixed <- dat[[1]]$y[, , , drop = FALSE]
# Detection Covariates
X.p <- dat[[1]]$X.p[, , , , drop = FALSE]
# Mixed design
for (t in 1:n.time) {
  y_mixed[which(rep.indx[, t] == 1), t, 2] <- NA
...
observation_data_full <- data.frame(
  X = dat[[1]]$coords[, 1] ,
  Y = dat[[1]]$coords[, 2] ,
  Observation = melt(dat[[1]]$y[,1,]), # select t=1
  SiteCov = dat[[1]]$X [,1,2],
  SiteCovW = dat[[1]]$w [,1],
  ObsCov = as.vector(dat[[1]]$X.p[,1,,2])
) 

# Reshape for plotting
observation_data_mixed <- data.frame(
  X = dat[[1]]$coords[, 1] ,
  Y = dat[[1]]$coords[, 2] ,
  Observation = melt(y_mixed[,1,]), # select t=1
  SiteCov = dat[[1]]$X [,1,2],
  SiteCovW = dat[[1]]$w [,1],
  ObsCov = as.vector(X.p[,1,,2])
) 


# Reshape for plotting
occupancy_data <- data.frame(
  X = rep(dat[[1]]$coords[,1], n.time),
  Y = rep(dat[[1]]$coords[,2], n.time),
  Time = rep(1:n.time, each = J),
  Occupied = as.vector(dat[[1]]$z)
)
...
proj_Z <- grid.arrange (
  ...
  data.frame (year=seq(1,n.time),
              Zt=colSums(dat[[1]]$z)/nrow(dat[[1]]$z)) %>%
    ggplot ()+
...
proj_X <- grid.arrange (
  ...
  # Plot the site covariate
  ggplot(dat[[1]]$coords %>%
           data.frame (var=dat[[1]]$X[,1,2]) , 
         aes(x = X1, y = X2, fill = var)) +
    geom_tile() +
    coord_fixed() +
    labs(title = "Site covariate (t=1)",
  ...
  ggplot(dat[[1]]$coords %>%
           data.frame (var=dat[[1]]$w[,1]) , 
         aes(x = X1, y = X2, fill = var)) +
    geom_tile() +
    coord_fixed() +
    labs(title = "Spatial Random Effect",
         x = "", y = "Latitude", fill = expression(omega[i]))+
    scale_colour_viridis_c(option = "magma",direction=1,na.value = "gray90")+
    scale_fill_viridis_c(option = "magma",direction=1,na.value = "gray90")+
    my_theme +
    theme(legend.position = "right",
  ...
  table(rowSums(is.na(y_mixed[,1,])!=T)) %>%
    data.frame () %>%
...
proj_Xp_Y <- grid.arrange (
  ...
  ggplot(observation_data_full,
         aes(x = X, y = Y, fill = ObsCov)) +
    geom_tile() +
    coord_fixed() +
    facet_wrap(~ Observation.X2 , nrow = 1) +
    labs(title = "Full ObsCov(t=1)",
         x = "Longitude", y = "Latitude", fill = expression(v[ij]))+
    scale_colour_viridis_c(option = "magma",
                           direction=1,na.value = "gray90",
                           limits=c(min(observation_data_mixed$ObsCov),
                                    max(observation_data_mixed$ObsCov)))+
  ...
  ggplot(observation_data_full, 
         aes(x = X, y = Y, fill = factor(Observation.value))) +
    geom_tile() +
    scale_fill_manual(values = c("black", "violet"),na.value = "gray90", 
  ...
  # MIXED DESIGN DATA
  ggplot(observation_data_mixed,
           aes(x = X, y = Y, fill = ObsCov)) +
    geom_tile() +
    coord_fixed() +
    facet_wrap(~ Observation.X2 , nrow = 1) +
    labs(title = "Mixed ObsCov",
         x = "Longitude", y = "Latitude", fill = expression(v[ij]))+
    scale_colour_viridis_c(option = "magma",
                           direction=1,na.value = "gray90",
                           limits=c(min(observation_data_mixed$ObsCov),
...
# arrange plot
png(here("figures", "sims_D&S", "proj_Y_DS.png"),
    #width = 30, height = 30, units = "cm", res=200)
...
# save
png(here("figures", "sims_D&S", "Figure-3.png"),
...
png(here("figures", "sims_D&S", "Density_sigma2.png"),
...
png(here("figures", "sims_D&S", "Density_phi.png"),
    width = 20, height = 20, units = "cm", res=200)
...
png(here("figures", "sims_D&S", "Density_sigma2_t.png"),
    width = 20, height = 20, units = "cm", res=200)
...
png(here("figures", "sims_D&S", "Density_rho.png"),
    width = 20, height = 20, units = "cm", res=200)
...
# sigma sq
png(here("figures", "sims_D&S", "biasVatT_sigma-sq.png"),
    width = 20, height = 20, units = "cm", res=200)
...
# phi
png(here("figures", "sims_D&S", "biasVatT_phi.png"),
    width = 20, height = 20, units = "cm", res=200)
...
# rho
png(here("figures", "sims_D&S", "biasVatT_rho.png"),
    width = 20, height = 20, units = "cm", res=200)
...
# sigma sq T
png(here("figures", "sims_D&S", "biasVatT_sigma-sqT.png"),
    width = 20, height = 20, units = "cm", res=200)
...
# arrange plot
png(here ("figures", "sims_D&S", "panel_sp.png"),
    width = 30, height = 30, units = "cm", res=200)

grid.arrange(
  check_spatial_patterns[[1]]$arranged_plot,
  check_spatial_patterns[[4]]$arranged_plot,
  corr.psi,
  corr.w,
  nrow=2,ncol=2
)
...
# plot 
png(here("figures", "sims_D&S", "Figure-3-spatial.png"),
    width = 20, height = 20, units = "cm", res=200)
...
png(here("figures", "sims_D&S", "Figure-3-temporal.png"),
    width = 20, height = 20, units = "cm", res=200)
...
# save
png(here("figures", "sims_D&S", "density.psi.p.png"),
    width = 20, height = 20, units = "cm", res=200)
...
# save
png(here("figures", "sims_D&S", "density.psi.p.bars.png"),
    width = 20, height = 20, units = "cm", res=200)
...
png(here("figures", "sims_D&S", "rel.beta0.beta1.png"),
    width = 20, height = 20, units = "cm", res=200)
...
# Distance of Sokal-Michener between simulated datasets
# Function to calculate Sokal-Michener distance between two binary vectors
sokal_michener_distance <- function(vec1, vec2) {
...
# extract y
each_y<-lapply (seq (1, 800), function (i)  cbind (melt(dat[[i]]$y),
                                                   scenario=i))
df_y <- (do.call(rbind,each_y))
# find the simulations with higher and lower p
lower_p <- psi_p_data[which(psi_p_data$beta0 == min (psi_p_data$beta0,na.rm=T)),]
lower_p <-  lower_p$sim+lower_p$scenario # sum of sim and scenario will show the dataset
higher_p <- psi_p_data[which(psi_p_data$beta0 == max (psi_p_data$beta0,na.rm=T)),]
higher_p <-  higher_p$sim+higher_p$scenario # sum of sim and scenario will show the dataset

# lower p
y1<-df_y[which(df_y$scenario == lower_p),]
y1$value<-as.logical(y1$value)
# higher psi
y2<-df_y[which(df_y$scenario == higher_p),]
y2$value<-as.logical(y2$value)
# distance between the more discrepant datasets
SM_distance_pair <- sokal_michener_distance(y1$value, y2$value)

# average distance across all data sets
SM_all <- lapply (unique(df_y$scenario), function (i) {
  
  # lower p
  y1<-df_y[which(df_y$scenario == i),]
  y1$value<-as.logical(y1$value)
  # higher psi
  y2<-df_y[which(df_y$scenario == i+1),]
  y2$value<-as.logical(y2$value)
  
  # jaccard index depicting similarity between data sets 
  SM_distance <- sokal_michener_distance(y1$value,y2$value)
  SM_distance
  
})


# Euclidean distance between simulated covariates
# Function to calculate Euclidean distance between two continuous vectors
...
# Euclidean distance between covariates of psi in the data sets with most discrepant beta0s
ED_pair <- euclidean_distance(melt(dat[[lower_p]]$X[,,2])[,3],
                              melt(dat[[higher_p]]$X[,,2])[,3]
  )

# average distance across all data sets
ED_all <- lapply (seq(2,length(unique(df_y$scenario))), function (i) {
  
  ED_all <- euclidean_distance(melt(dat[[i-1]]$X[,,2])[,3],
                               melt(dat[[i]]$X[,,2])[,3]
  )
...
# Euclidean distance between covariates of p in the data sets with most discrepant beta0s
ED_pair_p <- euclidean_distance(melt(dat[[lower_p]]$X.p[,,,2])[,4],
                              melt(dat[[higher_p]]$X.p[,,,2])[,4]
)

# average distance across all data sets
ED_all_p <- lapply (seq(2,length(unique(df_y$scenario))), function (i) {
  
  ED_all_p <- euclidean_distance(melt(dat[[i-1]]$X.p[,,,2])[,4],
                               melt(dat[[i]]$X.p[,,,2])[,4]
  )
...
# plot
png(here("figures", "sims_D&S", "DatasetDiffs.png"),
    width = 24, height = 20, units = "cm", res=200)

grid.arrange(
  
      # plot of comparison (y)
      ggplot (data.frame (SM=unlist(SM_all))) +
        geom_histogram(aes (x=SM),fill="gray",colour="black")+
        geom_vline(aes(xintercept = SM_distance_pair),size=1.2,linetype=2) +
        geom_vline(aes(xintercept = mean(SM)),size=1.2,linetype=1) +
        labs (x=bquote("Sokal-Michener distance of "*y[ijt]*" between data sets"),
              y="Number of data sets",
              title="A")+
        theme(plot.title=element_text(size=20))+my_theme,
      
      # plot of comparison (psi [it])
      ggplot (data.frame (ED=unlist(ED_all))) +
        geom_histogram(aes (x=ED),fill="gray",colour="black")+
        geom_vline(aes(xintercept = mean(ED)),size=1.2,linetype=1) +
        geom_vline(aes(xintercept = ED_pair),size=1.2,linetype=2) +
        labs (x=bquote("Euclidean distance between covariates of "*psi[it]*""),
              y="Number of data sets",
              title="B")+
        theme(plot.title=element_text(size=20))+my_theme,
      
      # plot of comparison (p [ijt])
      ggplot (data.frame (ED=unlist(ED_all_p))) +
        geom_histogram(aes (x=ED),fill="gray",colour="black")+
        geom_vline(aes(xintercept = mean(ED)),size=1.2,linetype=1) +
        geom_vline(aes(xintercept = ED_pair_p),size=1.2,linetype=2) +
        labs (x=bquote("Euclidean distance between covariates of "*p[ijt]*""),
              y="Number of data sets",
              title="C")+
        theme(plot.title=element_text(size=20))+my_theme,
...
# correlation of covariates of occupancy and detection
corr_sim_cov <- lapply(dat, function (i)

  cor(as.vector(apply (i$X.p[,,,2],c(1,2),mean)),
     as.vector(i$X[,,2]))
  
) 
# melt
corr_sim_cov_vector <- unlist(corr_sim_cov)

# plot
png(here("figures", "sims_D&S", "CovCorrelation.png"),
    width = 20, height = 20, units = "cm", res=200)

data.frame(rho=corr_sim_cov_vector ) %>%
  ggplot()+
  geom_histogram(aes(rho))+
  labs (x=bquote("Correlation between "*X[it]*" and aggregated "*S[it]*""),
        y="Number of data sets")+
  my_theme+
  geom_vline(aes(xintercept = mean(rho)),size=1.2,linetype=1) 

dev.off()
  
# end of D&S sims and interpretation