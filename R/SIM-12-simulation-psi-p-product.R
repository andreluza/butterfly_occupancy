...
prod_psi_det <- lapply (seq(1,n.sims), function (s) {
  lapply (seq (1, n.scenarios), function (sc) {
    ...
    # true product
    prod_occ_det <- scePhenSpotRev$psi.true [ , , s, sc] * apply(p_true, c(1,2), mean) # first two survey occasions
    ...
    for (t in 1:n.time) {
      for (i in 1:I) {
        for (j in 1:J) {
          ...
          # 0 in the logit scale will produce p = 0.5
          logit_p_hat[i, t, j] <- V_itj_L_i [i, t, j, ] %*% scePhenSpotRev$alpha.mean.samples[, s, sc]
    ...
    # estimated product
    est_occ_det <-  scePhenSpotRev$psi.mean.samples [ , , s, sc] * apply (p_hat, c(1,2), mean) 
...
fig.psi.p.prod <- plot.data[,-9] %>% # remove duplicated column
  ...
  facet_grid(time~spatial, 
             labeller = label_parsed)
# save
png(here("figures","TuningD&S_sims","ScenarioSpotRev","Figure-psi-p-st3-sc3.png"),
    width = 700, height = 600,units = "px")
