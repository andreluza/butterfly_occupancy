...
for (s in 1:n.sims) {
  ...
  for (sc in 1:n.scenarios) {
    ...
    # check
    # table(rowSums(D_it) == apply(rep.indx>1,1,sum,na.rm=T))
    ...
    # spatial gap?
    range(apply (y_mixed>=0,1,sum,na.rm=T))
    
    # Coordinates
    coords <- dat$coords
    
    # Package all data into a list
    occ.covs <- list(int = dat.full[[curr.indx]]$X[, , 1],
                     occ.cov.1 = dat.full[[curr.indx]]$X[, , 2])
    ...
    psi.mean.samples[, , s, sc] <- apply(out$psi.samples, c(2, 3), mean)
...
# ------------------------------------------------------
# illustrate the data produced by this scenario
...
# arrange plot
png(here("figures", "sims_present_paper", "Scenario0.png"),
