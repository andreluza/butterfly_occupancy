...
# Load the data sets
load(file = here ("model_output_mis-specification", "output_simulations", "sims_D&S", "sim-data-correct.rda"))

# load model output
load(file = here ("model_output_mis-specification", "output_simulations", "scenario_phenology_spot",
...
avg.df <- plot.df %>%
 ...
 summarize(val.avg = mean(val,na.rm=T),
	    est.avg = mean(est,na.rm=T)) %>%
 ungroup()
...
# save
png(here("model_output_mis-specification","figures", "Figure-3-st3-sc2-probit.png"),
